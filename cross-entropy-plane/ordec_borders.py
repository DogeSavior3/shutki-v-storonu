...
def plot_distributions(n, m, table=None, borders=None, lang='ru', ax=None, title='', color_palette=None, n_samples=20):
    if ax is None:
        ax = plt.gca()
    if borders is None:
    ...
    if table is None:
        ...
        if 'text_type' in my_table.columns:
            n_samples = min(len(my_table), n_samples)
    ...
    if 'text_type' in my_table.columns:
        if not color_palette:
            color_palette = {
                ...
                "gpt3": (1.0, 0.4980392156862745, 0.054901960784313725),  # orange
        ...
        my_table = my_table.sample(min(len(my_table), n_samples))
